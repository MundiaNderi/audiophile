      ...
      <div className="products justify-center leading-snug flex gap-5 flex-col md:flex-row">
        {suggestedProducts.map((product) => (
          <div key={product._id}>
            ...
            />
            <h1 className="text-lg text-center py-6 font-bold font-manrope">{product.name}</h1>
            <Link onClick={() =>
              console.log(`Navigating to: /product/${product._id}`)} className="mx-auto flex justify-center" to={`/product/${product._id}`}
            >
              <SeeProduct text="See Product" onClick={() => {
